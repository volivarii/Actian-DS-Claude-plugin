...
const {
  checkDirect,
  main,
} = require("../../plugins/actian-design-system/scripts/validation/check-direct.js");
...
describe("check-direct", () => {
  ...
  it("unknown-ds-class: the CLI reads the fragments the brief names", () => {
    ...
    const r = cp.spawnSync(process.execPath, [path.join(__dirname, "../../plugins/actian-design-system/scripts/validation/check-direct.js"), path.join(dir, "brief.json"), "--author", author], { encoding: "utf8" });
  ...
  it("unknown-ds-class: the CLI knows the classes of every fragment in the directory the brief indexes, not only the components it lists", () => {
    ...
    const runWith = (cls) => {
      ...
      return cp.spawnSync(process.execPath, [path.join(__dirname, "../../plugins/actian-design-system/scripts/validation/check-direct.js"), path.join(dir, "brief.json"), "--author", author], { encoding: "utf8" });
    };
