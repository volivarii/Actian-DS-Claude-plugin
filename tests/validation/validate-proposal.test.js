...
var { validateProposal, extractText, WORD_LIMITS } = require("../../plugins/actian-design-system/scripts/validation/validate-proposal.js");
...
describe("validateProposal (document)", function () {
  ...
  describe("option-width: a drawing is drawn at its own width, never squeezed", function () {
    ...
    it("keeps the bounds single-sourced with the renderer that draws them", function () {
      var DRAWING_WIDTH = require("../../plugins/actian-design-system/scripts/renderers/assemble-proposal.js").DRAWING_WIDTH;
  ...
  it("names the retired keys on a half-converted file carrying both shapes", function () {
    var d = load();
    d.approaches = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "fixtures", "proposal-dip-i-496-legacy.json"), "utf8")).approaches;
...
describe("validate-proposal.js CLI", function () {
  ...
  it("finds nothing at all in the three-decision acceptance document", function () {
    var acceptance = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "fixtures", "proposal-dip-i-496.json"), "utf8"));
  ...
  describe("the gaps between this file and what it says it covers", function () {
    ...
    it("names inputs the renderer genuinely throws on", function () {
      var assembleProposal = require("../../plugins/actian-design-system/scripts/renderers/assemble-proposal.js").assembleProposal;
...
describe("the evaluation stage", function () {
  var EVAL = path.join(__dirname, "..", "fixtures", "proposal-dip-i-496-evaluation.json");
...
describe("parts and words, held exactly", function () {
  var ACCEPTANCE = path.join(__dirname, "..", "fixtures", "proposal-dip-i-496.json");
  var EVAL = path.join(__dirname, "..", "fixtures", "proposal-dip-i-496-evaluation.json");
