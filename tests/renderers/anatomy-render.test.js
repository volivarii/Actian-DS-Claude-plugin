...
var ar = require("../../plugins/actian-design-system/scripts/lib/renderer.js").anatomyRender;

test("loadAnatomy: delegates to an injected loader with the slug", () => {
  var seen = null;
  var doc = { root: { id: "r" }, quality: { ratio: 1 } };
  var result = ar.loadAnatomy("read-only-tag", (slug) => {
    seen = slug;
    return doc;
  });
  assert.strictEqual(seen, "read-only-tag");
