...
var {
  ...
  path.resolve(
    __dirname, "..", "..", "plugins", "actian-design-system", "scripts", "renderers", "ds-coverage-report.js",
  ),
);
...
var { countBlankBoxes } = require(
  path.resolve(
    __dirname, "..", "..", "plugins", "actian-design-system", "scripts", "renderers", "renderability.js",
  ),
);
...
describe("blank-box budget", function () {
  it("POSITIVE CONTROL: the anatomy doc map is actually live", function () {
    ...
    if (unbuilt.length) {
      ...
      var docMap =
        require("../../plugins/actian-design-system/scripts/lib/renderer.js").dsAnatomyMap.buildDsAnatomyDocMap(
          r.slugs,
          { builtSlugs: [] },
        );
  ...
  it("banks only on a night the vendor changed, so a bank that no PR carries is never announced", function () {
    ...
    var wf = fs.readFileSync(
      path.resolve(
        __dirname, "..", "..", ".github", "workflows", "vendor-snapshot.yml",
      ),
      "utf8",
    );
...
describe("the built-slug record's own input", function () {
  it("POSITIVE CONTROL: measuring refuses when the vendored renderer exports no BUILT_SLUGS", function () {
    ...
    var dsMap = require(
      path.resolve(__dirname, "..", "..", "plugins", "actian-design-system", "scripts", "lib", "renderer.js"),
    ).dsHtmlMap;
