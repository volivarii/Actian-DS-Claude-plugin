...
var S = require("../../plugins/actian-design-system/scripts/fidelity/structural-check");
...
test("parseMetrics extracts the embedded JSON from dumped DOM", function () {
  var dom =
    '<div id=\'fidelity-metrics\'>{"overflow":false,"clipped":0,"absPos":0}</div>';
  assert.deepEqual(S.parseMetrics(dom), {
    overflow: false,
    clipped: 0,
    absPos: 0,
  });
...
test("verdict fails on absolute positioning (anti-rigid, v1 heuristic)", function () {
  ...
  assert.equal(v.pass, false);
  assert.ok(
    v.failures.some(function (f) {
      return f.kind === "abs-pos";
    }),
  );
});
...
describe("structural verdict fails on empty render", function () {
  it("an empty leaf at any width is a structural failure", function () {
    var v = S.verdict({
      360: { overflow: false, clipped: 0, absPos: 0, empty: true },
    });
  ...
  it("a clean non-empty leaf passes", function () {
    var v = S.verdict({
      360: { overflow: false, clipped: 0, absPos: 0, empty: false },
    });
    assert.strictEqual(v.pass, true);
  });
});

describe("measureScript detects empty renders", function () {
  it("includes getBoundingClientRect-based empty detection", function () {
    var s = S.measureScript();
