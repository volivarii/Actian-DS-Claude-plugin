...
var SKILL = path.resolve(__dirname, "../../plugins/actian-design-system/skills/actian-ux/SKILL.md");

/**
 * actian-ux-push-routing.test.js
 *
 * Asserts the companion router's HTML-first / opt-in-push contract introduced
 * in the actian-ux-prototype HTML-first simplification (v1.101.0+):
 *
 ...
 *
 * Matchers are keyed on text actually written into actian-ux/SKILL.md — they
...
describe("companion push-routing contract (HTML-first / opt-in push)", function () {
  ...
  it("row 1 (single-screen generate) routes to HTML deliverable only — no Figma push", function () {
    ...
    // /actian-ux-prototype without any push caveat).
    var row1Match = src.match(/\|\s*1\s*\|[^\n]+\|/);
    assert.ok(
      row1Match,
      "actian-ux/SKILL.md must contain a routing table row numbered 1",
    );
  ...
  it("row 2 (multi-screen generate) routes to HTML deliverable only — no Figma push", function () {
    ...
    assert.ok(
      row2Line,
      "actian-ux/SKILL.md must contain a routing table row numbered 2 (exact)",
    );
    var hasNoPush =
  ...
  it("explicit-Figma-prose row routes with --push (opt-in Figma push)", function () {
    ...
    assert.ok(
      row2aLine,
      "actian-ux/SKILL.md must contain routing row 2a for explicit-Figma-prose",
    );
    assert.ok(
  ...
  it("companion description line carries 'HTML-first' framing", function () {
    ...
    assert.ok(
      descMatch,
      "actian-ux/SKILL.md must have a description: frontmatter field",
    );
  ...
  it("row 8 (make it hifi) routes to actian-ux-prototype --hifi, never to convert-to-hifi", function () {
    var row8 =
      src.split("\n").filter(function (l) {
        return /^\| 8 \|/.test(l);
      })[0] || "";
    assert.ok(
      /\/actian-ux-prototype[^|]*--hifi/.test(row8),
      "Row 8 must route to /actian-ux-prototype with --hifi. Got: " + row8,
    );
    assert.ok(
  ...
  it("row 9 keeps only its actian-ux-prototype half", function () {
    var row9 =
      src.split("\n").filter(function (l) {
        return /^\| 9 \|/.test(l);
      })[0] || "";
    assert.ok(
      /\/actian-ux-prototype[^|]*--ref/.test(row9),
      "Row 9 routes to /actian-ux-prototype --ref. Got: " + row9,
    );
    assert.ok(
  ...
  // ─── actian-ux-proposal (2026-09-11) ───────────────────────────────────────────
  it("row 4 (alternatives / three ways) routes to actian-ux-proposal, not to --variants", function () {
    var row4 =
      src.split("\n").filter(function (l) {
        return /^\| 4 \|/.test(l);
      })[0] || "";
    ...
    assert.ok(
      /^\/actian-ux-proposal/.test(route4),
      "Row 4 must route to /actian-ux-proposal. Got: " + route4,
    );
    assert.ok(
  ...
  it("row 22 (approaches / concepts / options / proposal) routes to actian-ux-proposal with no push", function () {
    var row22 =
      src.split("\n").filter(function (l) {
        return /^\| 22 \|/.test(l);
      })[0] || "";
    assert.ok(
      /\/actian-ux-proposal X/.test(row22),
      "Row 22 must route to /actian-ux-proposal X. Got: " + row22,
    );
    assert.ok(
