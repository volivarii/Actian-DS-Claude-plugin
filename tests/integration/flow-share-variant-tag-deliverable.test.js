...
const assemble = require("../../plugins/actian-design-system/scripts/renderers/assemble-flow-share.js");
const anatomyRender = require("../../plugins/actian-design-system/scripts/lib/renderer.js").anatomyRender;
const appearanceRender =
  require("../../plugins/actian-design-system/scripts/lib/renderer.js").appearanceRender;
const { parseVariant } = require("../../plugins/actian-design-system/scripts/lib/renderer.js").dsHtmlMap;
...
// deliverable renders read-only-tag's per-variant colors from the appearance
...
// read-only-tag anatomy doc -- no hardcoded hex, no fixtures -- then compared
...
// renderer knowledge owns, so read-only-tag additionally emits a `ds-tag--<color>`
...
// removed the border from tags: read-only-tag's captured root appearance is now
...
// replaced read-only-tag's `Color` axis (7 values) with a `Type` axis (14
...
// Pick a `<prop>=<value>` variant string straight from read-only-tag's own
...
function pickColoredVariant(doc, base) {
  ...
  throw new Error(
    "read-only-tag's anatomy doc carries no appearance.variants entry whose " +
...
test("flow-share deliverable: read-only-tag renders per-variant colors from the appearance layer, keeps its instance label, and never injects a fallback-less var(--token)", () => {
  const doc = anatomyRender.loadAnatomy("read-only-tag");
  assert.ok(
    doc && doc.root,
    "read-only-tag anatomy doc must load (precondition)",
  );
  ...
  assert.ok(
    axisProp,
    "read-only-tag's anatomy doc must declare variantDefaults (precondition)",
  );
  ...
  if (colored.border && colored.border.color) {
    ...
    assert.ok(
      ...
      "the appearance layer captured no border for read-only-tag, so the deliverable " +
  ...
  // rendered most read-only-tag variants transparent/collapsed under path b.
