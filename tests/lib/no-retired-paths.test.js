...
const PLUGIN = "plugins/actian-design-system";

// git pathspecs, relative to the repository root: the plugin tree and the test
// tree (which lives outside the plugin so an install does not carry it), minus
// vendor (mirrors upstream), this test, descriptive docs, and release notes.
// Keep the excludes narrow; anything else MUST migrate.
const PATHSPECS = [
  PLUGIN,
  "tests",
  ":(exclude)" + PLUGIN + "/vendor",
  ":(exclude)tests/lib/no-retired-paths.test.js",
  ":(exclude)" + PLUGIN + "/CLAUDE.md",
  ":(exclude)" + PLUGIN + "/release-notes",
];
...
function liveReferencesTo(pattern) {
  const args = [
    ...
    "--",
    ...PATHSPECS.map((e) => JSON.stringify(e)),
  ].join(" ");
...
test("Task A3: path-b sidecar readers are gone from anatomy-render.js", () => {
  const src = fs.readFileSync(
    require("../../plugins/actian-design-system/scripts/lib/renderer.js").modulePath("anatomy-render.js"),
    "utf8",
  );
