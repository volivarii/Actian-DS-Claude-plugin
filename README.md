...
| `/design-proposal` | Propose: a reasoned document (the answer in one sentence, the terrain it touches, then one block per decision the ticket forces, each with two to four options drawn inside the surface the question lives on, a comparison, and a pick whose reasons name the row they argue from and whose cost says what ships with it, plus what is still open). HTML only, opens offline; for component-scale tickets. `--evaluate` stops after the decisions: what the ticket forces, written to `proposals/proposal-data.json` and no document, resumed with `--from`. The picks become a flow without being retyped: `/generate-flow --from proposals/proposal-data.json`. |
