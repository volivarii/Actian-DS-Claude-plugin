...
| `/design-proposal` | Propose: a reasoned document (a recommendation with reasons, then where the ticket lives today, what comparable products do, two to four approaches drawn inside the surface the ticket lives on, and a comparison). HTML only, opens offline; for component-scale tickets. |
