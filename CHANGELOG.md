...
- **`/generate-flow --direct`: one author draws the whole flow as a clickable prototype, checks it and looks at it.** ([#408](https://github.com/volivarii/Actian-DS-Claude-plugin/pull/408))
  ...
  route refuses `--push`, `--fm`, `--lofi`, `--audit`, `--variants`, `--breakpoints`, `--states`, `--from`,
  ...
  declared its own `proto` was told only that its step ids were empty; a focused control in a drawer scrolled the
  app header out of view. The measured acceptance run on the installed build decides whether `--direct` becomes
  the HTML default, in its own PR.
...
- **A drawing is drawn at its own width, and an option not chosen reads in one line**
  ...
  - **Screen and paper:** a drawing wider than the space it sits in scrolls inside its own box at
    ...
    wider than 620px is zoomed down to the page, so it prints whole and the prose around it keeps
    its size.
...
- **The overview view zooms** ([#360](https://github.com/volivarii/Actian-DS-Claude-plugin/pull/360)). Three buttons, 25, 50, and 100 percent, drive a `--ov-scale` custom
...
- **The use case defaults instead of asking a fourth question** ([#360](https://github.com/volivarii/Actian-DS-Claude-plugin/pull/360)). When neither keyword list matches,
...
- **Every step pointer resolves** ([#360](https://github.com/volivarii/Actian-DS-Claude-plugin/pull/360)). screen-generator's tier classification is a `## Step 0` heading,
...
- **Authoring notes for three leaves that render literals** ([#360](https://github.com/volivarii/Actian-DS-Claude-plugin/pull/360)). The HTML reference marks `fmDialog` and
