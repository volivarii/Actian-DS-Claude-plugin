...
function tokenGate(input) {
  ...
  var brokenRefFails = findings.filter(function (f) { return f.rule === "broken-ref"; }).length;
  var contrastFails = findings.filter(function (f) { return f.rule === "contrast"; }).length;
  ...
  return {
    ...
    errors: findings.filter(function (f) { return f.severity === "error"; }).length,
  };
}

module.exports = { rate: rate, tokenGate: tokenGate };
