...
// Fetch /nodes for each id in parallel via the wrapper's getNode (single-id
// per call). Wave 1 nightly volume is small enough that this is fine; if it
// becomes a hot spot, swap to a batched helper inside figma-rest.js.
function fetchNodesMap(rest, fileKey, ids) {
  ...
  return Promise.all(
    unique.map(function (id) {
      return rest.getNode(fileKey, id).then(function (resp) {
