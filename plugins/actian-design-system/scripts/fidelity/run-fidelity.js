...
function runStructural(slug, chrome, tmp) {
  ...
  S.WIDTHS.forEach(function (w) {
    var html = H.buildMeasureHtml(slug, frag, S.measureScript());
    var hp = path.join(tmp, slug + "-" + w + ".html");
    fs.writeFileSync(hp, html);
    per[w] =
      S.measureAtWidth({ chrome: chrome, htmlPath: hp, width: w }) ||
      { overflow: true, clipped: 0, absPos: 0 };
  });
...
function runPixel(slug, chrome, im, tmp, opts) {
  ...
  var stderr = P.imExec(im, P.buildCompareArgs({ a: rN, b: oN, fuzz: opts.fuzz || 2 }));
...
function ledgerRow(slug, gate1, gate2) {
  ...
  return {
    ...
    fidelity: {
      dimensions: {},
      score: gate1.pass && gate2.pass ? 1.0 : 0.0,
