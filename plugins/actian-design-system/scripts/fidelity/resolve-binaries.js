...
function defaultExists(p) {
  ...
  try { cp.execSync("command -v " + JSON.stringify(p), { stdio: "ignore" }); return true; }
...
function requireAll(resolved) {
  var missing = [];
  if (!resolved.chrome) missing.push("Chrome/Chromium (set CHROME_BIN, or install Google Chrome)");
  if (!resolved.imagemagick) missing.push("ImageMagick — run `brew install imagemagick` (macOS) or `apt-get install imagemagick`");
  if (missing.length) {
    throw new Error("[fidelity] missing required tools:\n  - " + missing.join("\n  - "));
  }
  return resolved;
}
...
module.exports = { resolveBinary: resolveBinary, resolveChrome: resolveChrome, resolveImageMagick: resolveImageMagick, requireAll: requireAll, resolveAll: resolveAll, defaultExists: defaultExists };
