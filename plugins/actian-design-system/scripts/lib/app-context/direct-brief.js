...
var LAYERS = {
  drawer: { width: 550, dock: "right edge, below the app header, no scrim, the page behind stays live" },
  panel: { width: 420, dock: "right edge of the content area, below the app header" },
...
function directBrief(brief, opts) {
  ...
  var components = Object.keys(found)
    ...
    .map(function (c) {
      return deps.exists(c.usageNotes) ? c : { slug: c.slug, fragment: c.fragment, usageNotes: null };
    });
  return {
    app: {
...
module.exports = { directBrief: directBrief, toDirect: toDirect, LAYERS: LAYERS };
