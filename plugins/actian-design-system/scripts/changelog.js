...
function diffComponentKeys(prevKeys, currKeys) {
  ...
  var added = currKeys.filter(function (k) { return !prevSet[k]; });
  var removed = prevKeys.filter(function (k) { return !currSet[k]; });
...
function buildChangelog(prevManifest, currSourceHash, currTokenHash, currComponentKeys) {
  var sourceChanged = prevManifest.sourceHash !== currSourceHash;
  var tokensChanged = prevManifest.tokenHash !== currTokenHash;
  var components = diffComponentKeys(prevManifest.componentKeys || [], currComponentKeys);
  var hasChanges = sourceChanged || tokensChanged || components.added.length > 0 || components.removed.length > 0;
...
function formatChangelog(result) {
  ...
  lines.push("  Source: " + (result.sourceChanged ? "changed (hash mismatch)" : "unchanged"));
  lines.push("  Tokens: " + (result.tokensChanged ? "changed (hash mismatch)" : "unchanged"));
  ...
  if (added.length === 0 && removed.length === 0) {
    ...
    lines.push("  Components: +" + added.length + " added, -" + removed.length + " removed");
    for (var i = 0; i < added.length; i++) {
      lines.push("    + " + resolveKeyName(added[i]) + " (" + added[i].substring(0, 8) + ")");
    }
    for (var j = 0; j < removed.length; j++) {
      lines.push("    - " + resolveKeyName(removed[j]) + " (" + removed[j].substring(0, 8) + ")");
    }
  }
...
if (require.main === module) {
  ...
  for (var a = 0; a < args.length; a++) {
    if (args[a] === "--previous" && args[a + 1]) { prevPath = args[++a]; }
    else if (args[a] === "--source" && args[a + 1]) { sourcePath = args[++a]; }
    else if (args[a] === "--tokens" && args[a + 1]) { tokensPath = args[++a]; }
    else if (args[a] === "--help") {
      process.stdout.write(JSON.stringify({
        name: "changelog",
        description: "Compare current push state against previous .last-push.json manifest",
        usage: "changelog.js --previous <.last-push.json> --source <data.json> --tokens <tokens.json>",
        flags: [
          { name: "--previous", description: "Path to existing .last-push.json" },
          { name: "--source", description: "Path to current source data file" },
  ...
  if (!prevPath || !sourcePath || !tokensPath) {
    process.stderr.write("Usage: changelog.js --previous <.last-push.json> --source <data.json> --tokens <tokens.json>\n");
    process.exit(1);
  }
  ...
  try {
    ...
    process.stderr.write("Error reading previous manifest: " + e.message + "\n");
    process.exit(1);
  }
  ...
  var result = buildChangelog(prevManifest, currSourceHash, currTokenHash, currComponentKeys);
