...
function screenHtml(s, index, apps) {
  if (!apps[s.app]) throw new Error('proposal-data: unknown app "' + s.app + '" in screen "' + s.id + '"; known: ' + Object.keys(apps).join(", "));
  var bad = unbalancedTag(s.html);
  if (bad) throw new Error('proposal-data: unbalanced <' + bad + '> in screen "' + s.id + '"');
  var chrome = s.chrome || "header";
  var strip = "";
  if (chrome === "header") {
    var type = flowRenderer.resolveChrome({ template: templateForApp(s.app) }).appHeaderType;
    strip = flowRenderer.appHeader(type);
  }
  var width = Number(s.width) || 360;
  return (
    '<div class="proposal-screen__col">' +
    '<span class="proposal-screen__label"><span class="proposal-screen__num">' + (index + 1) + "</span>" + esc(s.name) + "</span>" +
    '<div class="proposal-screen" data-name="' + esc(s.id) + '" style="width:' + width + 'px">' +
    strip +
    '<div class="proposal-screen__body">' + s.html + "</div>" +
    "</div>" +
    (s.caption ? '<p class="proposal-screen__caption">' + esc(s.caption) + "</p>" : "") +
    "</div>\n"
  );
}

function assembleProposal(data) {
  var errors = validateSchema(data, JSON.parse(fs.readFileSync(SCHEMA_PATH, "utf8"))).filter(function (e) { return e.indexOf("(warning)") === -1; });
  if (errors.length) throw new Error("proposal-data: schema: " + errors.join("; "));
  var meta = data.meta;
  var apps = loadApps();
  var screens = data.screens.map(function (s, i) { return screenHtml(s, i, apps); }).join("");
  var context = [meta.ticket || "", (meta.apps || []).map(function (a) { return apps[a] && apps[a].label ? apps[a].label : a; }).join(", "), meta.date].filter(Boolean).join("  ·  ");
  var recommendation = meta.recommendation ? '  <section class="board__recommendation"><h2>Recommendation</h2>' + meta.recommendation + "</section>\n" : "";
  var metaComment =
  ...
  return template
    ...
    .replace("<!-- {{SCREENS}} -->", function () { return screens; })
    .replace("{{RECOMMENDATION}}", function () { return recommendation; });
}

module.exports = { assembleProposal: assembleProposal, extractUnbalancedTag: unbalancedTag };
