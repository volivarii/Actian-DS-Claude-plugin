...
(function () {
  ...
  // Shared structural-node renderer (single source of truth). Resolved the
  // same way as fm-html-map; presentation uses the Roboto default font.
  var nodeRenderer =
  ...
  function renderCover(slide) {
    return (
      '<div class="slide slide--cover" data-name="' +
      esc("Slide: " + slide.title) +
      '">' +
      geoPattern +
      '<div class="slide__topic">' +
      esc(slide.topic || "") +
      "</div>" +
      ...
      "</div>" +
      (slide.subtitle
        ? '<div class="slide__subtitle">' + esc(slide.subtitle) + "</div>"
        : "") +
  ...
  function renderSection(slide) {
    return (
      '<div class="slide slide--section" data-name="' +
      esc("Section: " + slide.title) +
      '">' +
      ...
      '<div class="geo-rect geo-rect--3"></div>' +
      "</div>" +
      '<div class="slide__section-topic">' +
      esc(slide.topic || "") +
      "</div>" +
      '<div class="slide__section-title">' +
      esc(slide.title) +
      "</div>" +
      "</div>"
    );
  ...
  function renderBodyFull(slide) {
    ...
    if (slide.content && Array.isArray(slide.content)) {
      contentHtml = slide.content
        .map(function (el) {
          return renderContentElement(el);
        })
        .join("");
    ...
    return (
      '<div class="slide slide--body" data-name="' +
      esc("Slide: " + slide.title) +
      '">' +
      '<div class="slide__title-bar">' +
      esc(slide.title) +
      "</div>" +
      '<div class="slide__content-full">' +
      contentHtml +
      "</div>" +
      "</div>"
    );
  ...
  function renderBodyTextVisual(slide) {
    ...
    return (
      ...
      '<div class="slide__text-col">' +
      textHtml +
      "</div>" +
      '<div class="slide__visual-col">' +
      visHtml +
      "</div>" +
      "</div></div>"
    );
  ...
  function renderBackCover(slide) {
    return (
      '<div class="slide slide--cover" data-name="Back cover">' +
      geoPattern +
      '<div class="slide__back-title">' +
      esc(slide.text || "Thank you") +
      "</div>" +
      (slide.subtitle
        ? '<div class="slide__back-subtitle">' + esc(slide.subtitle) + "</div>"
        : "") +
      "</div>"
    );
  }
  ...
  // renderSpecNode — structural Figma-node → HTML. Now a thin wrapper over the
  // shared render-node.js module (presentation uses the Roboto default font).
  // Structural cases (FRAME/TEXT/RECT/DIVIDER + the newly-gained INSTANCE/
  // ELLIPSE) live in the shared superset; presentation-specific chart/table
  // elements remain in renderContentElement below.
  function renderSpecNode(node) {
    return nodeRenderer.renderNode(node, { defaultFont: "Roboto" });
  }

  function renderContentElement(el) {
    switch (el.type) {
      case "stat-cards":
        return renderStatCards(el.cards);
      case "bar-chart":
        return renderBarChart(el);
      case "progress-bars":
        return renderProgressBars(el.bars);
      case "comparison-table":
        return renderComparisonTable(el);
      case "timeline":
        return renderTimeline(el.events);
      case "html":
        return el.html || "";
      ...
      cards
        .map(function (c) {
          return (
            ...
            "</div>" +
            (c.trend
              ? '<div class="stat-card__trend">' + esc(c.trend) + "</div>"
              : "") +
            "</div>"
          );
        })
  ...
  function renderBarChart(chart) {
    ...
    return (
      '<div class="bar-chart">' +
      (chart.title
        ? '<div class="bar-chart__title">' + esc(chart.title) + "</div>"
        : "") +
      chart.bars
        .map(function (b, i) {
          ...
          return (
            ...
            esc(b.label) +
            "</div>" +
            '<div class="bar-chart__track"><div class="bar-chart__fill bar-chart__fill--' +
            ((i % 9) + 1) +
            '" style="width:' +
            pct +
            '%;"></div></div>' +
      ...
      bars
        .map(function (b) {
          return (
            ...
            b.percent +
            "%</span></div>" +
            '<div class="progress-bar__track"><div class="progress-bar__fill" style="width:' +
            b.percent +
            '%;"></div></div>' +
            "</div>"
          );
      ...
      (events || [])
        .map(function (ev) {
          return (
            ...
            "</div>" +
            (ev.description
              ? '<div class="timeline__desc">' + esc(ev.description) + "</div>"
              : "") +
            "</div>"
          );
        })
  ...
  function renderSlide(slide) {
    switch (slide.type) {
      ...
      case "body-full":
        return renderBodyFull(slide);
      case "body-text-visual":
        return renderBodyTextVisual(slide);
      case "back-cover":
        return renderBackCover(slide);
      default:
        return renderBodyFull(slide);
    }
  }
