    ...
    req.on('end', () => {
      try {
        ...
        fs.writeFile(outPath, JSON.stringify(data, null, 2), (err) => {
          ...
          sendJSON(res, 200, { ok: true, file: outPath, count: data.annotations.length });
        });
      } catch (e) {
...
server.listen(port, () => {
  process.stderr.write(`Preview server listening on http://localhost:${port} serving ${dirAbs}\n`);
});
