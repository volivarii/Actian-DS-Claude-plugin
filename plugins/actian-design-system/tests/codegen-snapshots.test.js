...
for (const test of TESTS) {
  ...
  if (!fs.existsSync(snapshotDir)) {
    console.error(`FAIL [${test.name}] No snapshots found. Run with UPDATE_SNAPSHOTS=1 first.`);
    failed++;
    continue;
  }

  const baselineManifest = JSON.parse(fs.readFileSync(path.join(snapshotDir, 'manifest.json'), 'utf8'));

  // Check call count
  if (manifest.totalCalls !== baselineManifest.totalCalls) {
    console.error(`FAIL [${test.name}] Call count changed: ${baselineManifest.totalCalls} → ${manifest.totalCalls}`);
    failed++;
    continue;
  }
  ...
  for (const call of manifest.calls) {
    ...
    if (!fs.existsSync(baselinePath)) {
      console.error(`FAIL [${test.name}] New call file: ${call.file} (not in baseline)`);
      testPassed = false;
      continue;
    }
    ...
    if (actual !== baseline) {
      ...
      let diffLine = -1;
      for (let i = 0; i < Math.max(actualLines.length, baselineLines.length); i++) {
      ...
      console.error(`FAIL [${test.name}] ${call.file} differs at line ${diffLine} (size ${sizeDiff > 0 ? '+' : ''}${sizeDiff} bytes)`);
      if (diffLine > 0) {
        console.error(`  baseline: ${(baselineLines[diffLine - 1] || '').substring(0, 100)}`);
        console.error(`  actual:   ${(actualLines[diffLine - 1] || '').substring(0, 100)}`);
      }
      testPassed = false;
    }
  }
  ...
  if (testPassed) {
    console.log(`PASS [${test.name}] ${manifest.totalCalls} call(s), all match`);
    passed++;
  } else {
    failed++;
  }
