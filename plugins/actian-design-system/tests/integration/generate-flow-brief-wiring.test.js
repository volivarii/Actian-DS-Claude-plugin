...
const skill = fs.readFileSync(path.join(PLUGIN_ROOT, "skills/generate-flow/SKILL.md"), "utf8");
const agent = fs.readFileSync(path.join(PLUGIN_ROOT, "agents/screen-generator.md"), "utf8");
...
describe("generate-flow wires the brief", () => {
  ...
  it("the agent reads the brief and no longer opens app-context.json or runs the inspector by default", () => {
    ...
    assert.doesNotMatch(agent, /read `vendor\/app-context\/dist\/app-context\.json`/);
