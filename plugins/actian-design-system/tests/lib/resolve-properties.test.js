...
var resolver = require(
  path.join(PLUGIN_ROOT, "scripts", "lib", "app-context", "resolve-properties.js"),
);
...
describe("resolve-properties (resolver core)", function () {
  it("resolves data-product's properties as normalized {name,label,type} objects", function () {
    ...
    assert.ok(Array.isArray(props) && props.length >= 8, "expected data-product props");
    ...
    assert.ok(names.indexOf("name") !== -1, "data-product → name field expected");
  });

  it("preserves the typed shape (enum + states) carried forward for S3c", function () {
    var props = resolver.resolveProperties("data-product");
    var status = props.filter(function (p) { return p.name === "status"; })[0];
  ...
  it("humanizeName sentence-cases space-separated phrases", function () {
...
describe("resolve-properties (mixed string/object array — synthetic, injection seam)", function () {
  ...
  it("normalizes both string and object entries; drops invalid ones; carries states/example", function () {
    ...
    assert.deepStrictEqual(props[0], { name: "foo", label: "Foo", type: "string" });
...
describe("resolve-properties (CLI)", function () {
  ...
  var CLI = path.join(PLUGIN_ROOT, "scripts", "lib", "app-context", "resolve-properties.js");

  it("--entity data-product prints { entity, properties } and exits 0", function () {
    var out = execFileSync(NODE, [CLI, "--entity", "data-product"], { encoding: "utf8" });
    var parsed = JSON.parse(out);
    assert.strictEqual(parsed.entity, "data-product");
    assert.ok(Array.isArray(parsed.properties) && parsed.properties.length >= 8);
  });

  it("--entity <unknown> exits 1", function () {
    assert.throws(function () {
      execFileSync(NODE, [CLI, "--entity", "nope"], { encoding: "utf8", stdio: "pipe" });
    });
  });
});
