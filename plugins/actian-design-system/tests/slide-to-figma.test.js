...
if (Array.isArray(result) && result.length > 1) {
  ...
  process.stdout.write(
    "  (skipped — fixture fits in single call, __WRAPPER_ID__ not needed)\n",
  );
...
if (Array.isArray(result)) {
  ...
  for (const slideName of slideNames) {
    assertContains(
      allCode,
      slideName,
      slideName + " appears in generated code",
    );
  }
}
...
if (Array.isArray(result) && result.length > 0) {
  ...
  assertContains(
    ...
    "Call 1 sets skill to generate-presentation",
  );
}
...
if (Array.isArray(result) && result.length > 0) {
  ...
  assertContains(
    ...
    "Call 1 imports variables via Figma API",
  );
  assertContains(call1Code, "bgDefault", "Call 1 imports bgDefault variable");
  assertContains(
    call1Code,
    "textReverse",
    "Call 1 imports textReverse variable",
  );
}
...
if (Array.isArray(result)) {
  ...
  assertContains(
    allCode,
    "GRADIENT_LINEAR",
    "Gradient fills appear in generated code",
  );
  assertContains(
    allCode,
    "angleToTransform",
    "Angle transform used for gradient",
  );
  assertContains(
    allCode,
    "#090952",
    "Dark gradient stop color appears in code",
  );
}
...
if (Array.isArray(result)) {
  ...
  assertNotContains(
    allCode,
    "buildFromSpec",
    "No buildFromSpec (interpreter) reference",
  );
  assertNotContains(
    allCode,
    "figma-interpreter",
    "No figma-interpreter reference",
  );
}
...
const os = require("os");
const fs = require("fs");
const outputDir = path.join(os.tmpdir(), "slide-to-figma-test-" + Date.now());
