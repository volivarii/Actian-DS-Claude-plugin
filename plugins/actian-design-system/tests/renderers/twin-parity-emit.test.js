...
var EMITTER = path.resolve(__dirname, "../../scripts/renderers/html-renderers/render-node-figma.js");
...
["form-create", "table-list"].forEach(function (name) {
  describe("twin-parity emit — " + name, function () {
    it("emitted Plugin-API code matches golden", function () {
      ...
      var r = cp.spawnSync(process.execPath, [EMITTER, "--parent-id", "1:1"], { input: spec, encoding: "utf8" });
      assert.equal(r.status, 0, r.stderr);
      var goldenPath = path.join(FX, name + ".golden.js");
      if (UPDATE) { fs.writeFileSync(goldenPath, r.stdout); return; }
