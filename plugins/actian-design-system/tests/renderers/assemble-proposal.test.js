...
var {
  assembleProposal,
} = require("../../scripts/renderers/assemble-proposal.js");
...
describe("assembleProposal", function () {
  ...
  it("renders one screen per entry, at its width, with its label and caption", function () {
    var data = load();
    assert.strictEqual(
      html.split('class="proposal-screen"').length - 1,
      data.screens.length,
    );
    data.screens.forEach(function (s, i) {
      assert.ok(
        html.indexOf('style="width:' + s.width + 'px"') !== -1,
        s.id + " width",
      );
      assert.ok(
        html.indexOf(">" + (i + 1) + "</span>") !== -1,
        s.id + " number",
      );
      assert.ok(html.indexOf(s.caption) !== -1, s.id + " caption");
      assert.ok(
        html.indexOf('data-name="' + s.id + '"') !== -1,
        s.id + " data-name",
      );
    });
  });
  it("renders the app header strip through the flow renderer's own appHeader markup", function () {
    ...
    assert.strictEqual(
      html.split(admin).length - 1,
      1,
      "one Administration strip",
    );
    assert.strictEqual(
      html.split(explorer).length - 1,
      2,
      "two Explorer strips",
    );
  });
  it("omits the strip when chrome is none", function () {
    ...
    assert.strictEqual(
      out.split(flowRenderer.appHeader("Administration")).length - 1,
      0,
    );
  });
  it("prints the authored date and the recommendation, never a render-time clock", function () {
    ...
    assert.ok(
      html.indexOf("Ship the badge as the default") !== -1,
      "recommendation",
    );
    assert.strictEqual(
      assembleProposal(load()),
      html,
  ...
  it("carries the single data-toggle listener", function () {
    assert.strictEqual(
      html.indexOf('data-toggle="menu-default"') !== -1,
      true,
      "the fragment's data-toggle survived",
    );
    assert.strictEqual(
      html.split("el.hidden = !el.hidden").length - 1,
      1,
      "one listener",
    );
  });
  it("escapes titles and names (no attribute breakout)", function () {
    ...
    assert.ok(out.indexOf("&lt;b&gt;") !== -1, "name escaped");
    assert.ok(out.indexOf("<b>") === -1, "no unescaped name leak");
  });
  it("refuses an unknown app slug, a schema error and an unbalanced fragment", function () {
    var a = load();
    a.screens[0].app = "not-an-app";
    assert.throws(function () {
      assembleProposal(a);
    }, /proposal-data: unknown app "not-an-app"/);
    var b = load();
    delete b.screens[0].html;
    assert.throws(function () {
      assembleProposal(b);
    }, /proposal-data: schema/);
    var c = load();
    c.screens[0].html = "<div><div>open</div>";
    assert.throws(function () {
      assembleProposal(c);
    }, /proposal-data: unbalanced <div> in screen "create-group"/);
  });
  it("refuses an unbalanced recommendation", function () {
    var d = load();
    d.meta.recommendation = "<p>Ship it</p><div>unclosed";
    assert.throws(function () {
      assembleProposal(d);
    }, /proposal-data: unbalanced <div> in recommendation/);
  });
});
...
describe("assemble-preview.js --type proposal", function () {
  it("writes the board through the shared CLI", function () {
    var out = path.join(
      require("os").tmpdir(),
      "proposal-" + Date.now() + ".html",
    );
    var r = spawnSync(
      process.execPath,
      [
        path.join(ROOT, "scripts", "renderers", "assemble-preview.js"),
        FIXTURE,
        "--type",
        "proposal",
        "-o",
    ...
    assert.ok(
      fs.readFileSync(out, "utf8").indexOf('class="proposal-screen"') !== -1,
    );
  });
  it("lists proposal among --help types", function () {
    ...
    assert.ok(
      help.types.indexOf("proposal") !== -1,
      JSON.stringify(help.types),
    );
  });
});
