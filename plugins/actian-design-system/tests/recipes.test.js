...
const VALID_NODE_TYPES = new Set(['FRAME', 'TEXT', 'INSTANCE', 'DIVIDER', 'ELLIPSE', 'RECT']);
...
function validateContentNode(node, nodePath) {
  ...
  if (!node.type || !VALID_NODE_TYPES.has(node.type)) {
    errors.push(`${nodePath}: invalid type "${node.type}" (expected one of ${[...VALID_NODE_TYPES].join(', ')})`);
  }
  ...
  if (node.spacing !== undefined && !SPACING_SCALE.has(node.spacing)) {
    errors.push(`${nodePath}: spacing ${node.spacing} not in scale [${[...SPACING_SCALE].join(', ')}]`);
  }
  if (node.paddingX !== undefined && !SPACING_SCALE.has(node.paddingX)) {
    errors.push(`${nodePath}: paddingX ${node.paddingX} not in scale [${[...SPACING_SCALE].join(', ')}]`);
  }
  if (node.paddingY !== undefined && !SPACING_SCALE.has(node.paddingY)) {
    errors.push(`${nodePath}: paddingY ${node.paddingY} not in scale [${[...SPACING_SCALE].join(', ')}]`);
  }
...
for (const entry of index) {
  ...
  // ── index-level checks ──
  if (typeof entry.file !== 'string') errors.push('index entry missing "file" (string)');
  if (typeof entry.archetype !== 'string') errors.push('index entry missing "archetype" (string)');
  if (typeof entry.pattern !== 'number') errors.push('index entry missing "pattern" (number)');
  if (!Array.isArray(entry.tags)) errors.push('index entry missing "tags" (array)');
  ...
  if (typeof recipe.slots !== 'object' || recipe.slots === null || Array.isArray(recipe.slots)) {
  ...
  // ── archetype match ──
  if (typeof recipe.archetype === 'string' && recipe.archetype !== entry.archetype) {
    errors.push(`archetype mismatch: index says "${entry.archetype}", recipe says "${recipe.archetype}"`);
  }
