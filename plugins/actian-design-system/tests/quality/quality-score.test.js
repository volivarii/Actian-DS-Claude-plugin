...
describe("tokenGate", function () {
  it("is 1.0 when there are no findings", function () {
    var g = q.tokenGate({ findings: [], counts: { refsChecked: 50, contrastChecks: 12 } });
  ...
  it("computes per-type pass-rates from findings and counts", function () {
    var findings = [
      ...
      { rule: "contrast", severity: "error", token: "link [studio]", message: "" },
      { rule: "contrast", severity: "error", token: "link [explorer]", message: "" },
    ];
    var g = q.tokenGate({ findings: findings, counts: { refsChecked: 50, contrastChecks: 12 } });
  ...
  it("treats a zero denominator as a full pass (no checks = nothing failing)", function () {
    var g = q.tokenGate({ findings: [], counts: { refsChecked: 0, contrastChecks: 0 } });
    assert.strictEqual(g.score, 1);
  });
});
