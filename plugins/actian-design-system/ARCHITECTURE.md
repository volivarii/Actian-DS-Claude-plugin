...
Retired and deleted: `/generate-presentation`, `/convert-to-hifi` and `agents/slide-generator.md` (retired 2026-09-10), `/compare-flows`, `/component-brief`, `/create-component` and `agents/{brief-researcher,card-generator,brief-data-validator,parity-analyzer}.md` (retired 2026-09-21, roadmap 711). Their code left the repository on 2026-09-22 (plugin 2026.9.60): the Python Office renderer and its brand assets, the brief and presentation recipes, schemas, renderers and references, the figma-table renderer, the eval lane, the six `scripts/lib/` modules only the brief renderer required, the brief transcription module (its one live function, `isStubGuideline`, moved to `scripts/lib/stub-guideline.js` for the flow validator), and `retired/` itself. Git history is the restore point (`git log --diff-filter=D`). `scripts/transformers/category-defaults-loader.js` stays because the accessibility resolver reads it.
