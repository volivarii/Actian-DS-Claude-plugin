...
- `quality/` — DS Quality Score (`quality-score.js` + `quality-score-cli.js`): composite headline (token-lint + fidelity rollup) → `quality-ledger.jsonl`; run report-only in `pr-checks.yml`. New cross-gate quality-aggregation scripts go here.
...
- `sync/`, `validation/`, `renderers/`, `transformers/`, `foundations/`, `changelog/`, `lib/`, `lint/`, `quality/` — unit tests for the corresponding `scripts/<bucket>/` modules.
