...
| `/generate-flow` | `skills/generate-flow/SKILL.md` | `agents/{screen-generator,flow-consistency,flow-researcher,wiring-analyzer,parity-analyzer}.md` | `recipes/flow/` | `templates/flow-*.html` (incl. `flow-prototype-wrapper.html` — two-view Prototype + Overview shell for `flow-share`); `templates/vendor/alpinejs-3.14.9.min.js` (MIT, inlined into `flow-share` deliverable) | (none) | `references/generate-flow/` (incl. `share.md` — `--share` self-contained shareable deliverable) | `references/figma/*`, `references/ds-rules/{layout-patterns,quality-checklist,quality-tiers}.md`, `references/context/{app-context,ux-patterns}.md` |
