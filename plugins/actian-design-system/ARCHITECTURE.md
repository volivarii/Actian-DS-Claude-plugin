...
- `quality/` — DS Quality Score (`quality-score.js` + `quality-score-cli.js`): composite headline (token-lint + fidelity rollup) → `quality-ledger.jsonl`; run report-only in `pr-checks.yml`. `quality-gates-cli.js` runs the live Chrome composite (token + structural fidelity) for the `quality-gates` job. New cross-gate quality-aggregation scripts go here.
