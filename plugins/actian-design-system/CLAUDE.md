...
- `vendor/foundations/src/NN-<slug>.md` per-section files (00-intro, 01-table-of-contents, 02-color-primitives, 03-tokens, 04-design-guidelines, 05-handoff-protocol, 06-related-guidelines) + `vendor/foundations/dist/*.json` (8 derived) + `vendor/foundations/src/AUTHORING.md`
- `vendor/tokens/tokens.json` (DTCG, 3 themes) + `vendor/tokens/tokens.css` + `vendor/tokens/token-reference.md`
- `vendor/accessibility/src/NN-<slug>.md` (per-section files: principles, color-contrast, typography, motion, focus-keyboard, aria-labels, reading-order-landmarks, touch-pointer, error-prevention, session-timeout-warnings, components, designer-handoff-checklist)
...
- **Accessibility:** `vendor/accessibility/src/NN-<slug>.md` (per-section) — WCAG 2.1 AA
