...
- `vendor/components/dist/guidelines/<slug>.json` — merged per-component multi-domain guideline docs (`domains.{content,usage,design,behavior,tokens}`, each with a `status`). Resolve by slug via `PATHS.components.guidelineDoc.byKey(slug)`; knowledge ships registry-key alias copies (e.g. `dropdown-select-default.json` carries `_alias_of: "dropdown-select"`) so registry slugs resolve directly. `guidelines.bundle.json` is the one-shot roll-up. *(The legacy scraped guideline layer under `components/src/guidelines/` was retired in Phase 5, knowledge v0.11.0.)*
...
- `scripts/validation/validate-flow-data.js` — pipeline validation (banned text, tokens, terminology) + `avoid-word` soft-check (warnings, non-blocking; `--skip-avoid-words`) against `vendor/content/dist/words-to-avoid.json`, plus non-blocking **substrate-grounding advisories** (`chrome-*`, `pattern-ungrounded`, `relationships-ungrounded`, `properties-ungrounded`, `section-ungrounded`, `enum-not-typed` — all `info`, flow-level `screen:""`, NOT in `HARD_KINDS`; new finding kinds must also be registered in `CLI_VISIBLE_KINDS`). Note: flow-data is **not** ajv-validated — schema checks go through the hand-rolled `scripts/validation/validate-schema.js` (no `oneOf`).
- `scripts/transformers/fm-tree-to-flow-data.js` — converts FM Figma tree to flow-data.json
- `scripts/bridges/proposal-to-flow.js`: reads a `proposals/proposal-data.json` and composes it into the seed `/actian-ux-prototype` takes: `compose(data, opts)` returns `{ screens, brief, findings }`, and a CLI prints or writes it. Screens sharing a name merge into one carrying every note; sources that disagree on a merged screen's `template`, `app` or `entity` are a P0 and the bridge emits nothing. The emitted `template` is the CHROME name (the screen's app), not the proposal's flow archetype, which rides along as `archetype`: the two fields share a name and do not share a vocabulary, and a seed carrying the archetype in `template` renders no chrome.
