...
- `vendor/components/src/guidelines/*.json` — **transitional** Figma-scraped guideline layer (carries `_stub`, `content_guidelines`, `behavior`). Still read by `validate-flow-data.js`; retired in the guideline-architecture Phase 5. New code should consume `components/dist/guidelines/` instead.
...
- `vendor/content/dist/global.md` — global / cross-cutting content guidelines (voice, tone, words to avoid, UX-pattern topics). Component-specific copy lives per-component in `components/dist/guidelines/<slug>.json` `domains.content`.
- `vendor/content/dist/content.md` — **transitional** full content concat (global + per-component); retired in Phase 5. New code should consume `global.md` + the per-component guideline docs.
