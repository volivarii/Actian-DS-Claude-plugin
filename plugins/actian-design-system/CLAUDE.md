...
**Key data files** (all vendored from `volivarii/actian-ds-knowledge`):
- `vendor/components/registries/{fmkit,dskit,metakit}.json` — component registries (keys, variants, properties, defaults)
- `vendor/components/guidelines/*.json` — 85 component guidelines (44 curated + 41 auto-stubs; stubs carry `_stub: true`)
- `vendor/components/{fm,dskit}-components.md` + `vendor/components/meta-kit/components.md` — human-readable mirrors regenerated by `render-component-reference.js` post-vendor-pull
- `vendor/components/{text,effect}-styles.md` — DS Kit text + effect style references (auto-regenerated by knowledge-repo CI)
- `vendor/foundations/foundations.md` + `vendor/foundations/*.json` (8 derived) + `vendor/foundations/AUTHORING.md`
