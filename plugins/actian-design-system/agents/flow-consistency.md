...
Scan all visible text in the HTML for terminology violations. Load `vendor/app-context/dist/app-context.json` → `terminology` section. For each term, check if any `notUse` values appear in visible text (case-insensitive word-boundary match):
