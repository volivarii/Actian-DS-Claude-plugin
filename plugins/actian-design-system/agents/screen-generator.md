...
- **Layers and links:** a slice with `screen.layer` is a surface over screen `screen.layer.over`: write the layer body only, never that base or its chrome (merge sets `layer`). Put `goto` on the primary action, set to the next entry's `id` in the slice's `flow` (none on the last screen). Never emit `screen.id`: merge stamps the ids `flow` lists. Examples: html-reference.md, Layered screens.
