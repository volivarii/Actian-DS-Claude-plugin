...
A slice with `screen.layout === "freehand"` is `improvised` with the justification `freehand layout requested`: `prepare-flow.js` already left `archetype` and `pageRecipe` null on such a slice, so there is no fit check to run, only the layout tests the flag was set up for. A slice with `screen.layer` and no `pattern` is the same: `archetype` and `pageRecipe` are null, and the body composes from `screen.layer.kind` alone (drawer, panel, modal or toast), never a page skeleton.
...
- **Glossary:** use `glossary` as the single source for entity names in page headers/breadcrumbs/body text, and action verbs in button labels/CTAs. Never invent alternative phrasings for glossary terms.
- **Sidebar nav:** Never write `navItems`, `activeNavItem` or `sidebar`. Merge stamps the rail and its active item on every screen from the screen list, and removes what an agent wrote.
...
- **Layers and links:** a slice with `screen.layer` is a surface over screen `screen.layer.over`: write the layer body only, never that base or its chrome (merge sets `layer`). When the slice carries `screen.exit`, put `goto: "<screen.exit.toId>"` on the element `screen.exit.via` names, and make sure that element exists: it is how the user leaves this screen. When it is a row or a control inside a leaf (a table row, a page-header action), put `goto` on the leaf INSTANCE itself. No `screen.exit` means no onward link is owed. Other links (Cancel back to the base) aim at an `id` in the slice's `flow`. Never write `exit`. Never emit `screen.id`: merge stamps the ids `flow` lists. Examples: html-reference.md, Layered screens.
