...
- **Sidebar nav (grounded):** every screen that shows the app shell MUST set `navItems` from `meta._glossary.chrome.sidebar` — the same labels, in the same order. Mark the current location with `state: "On"` on the matching item; leave the others unset. Do **not** add, remove, rename, or reorder items — that is a flow-level decision already recorded in `meta._glossary.chrome` (with `chromeJustification`). On a focused screen that suppresses the shell (full-page wizard, modal-first, empty-first), omit `navItems`.
