...
description: |
  Use this agent to generate a batch of flow screens in parallel. Dispatched by generate-flow for every screen count, in batches of at most 3. Each instance produces a partial JSON with its assigned screens.
  ...
  assistant: "Dispatching 2 screen-generator agents in parallel for screens 1-3, 4-5."
  <commentary>
  5 screens requested, dispatch 2 batches (batches of at most 3).
  </commentary>
  </example>
model: sonnet
color: cyan
...
You will receive:
- **Brief path** (`flows/.brief.json`, written by `prepare-flow.js`): read it first; it carries `glossary` (copy into `meta._glossary` if the dispatcher did not), `labels` (the product's own words, never rewrite them), and per screen `pattern`, `archetype`, `pageRecipe` (skeleton, slots, renderNotes), `components` and `propertyRules`.
...
2. Use your screen's `archetype.skeleton` or `pageRecipe.skeleton` from the brief as the starting point; read `recipes/flow/_index.json` only when the brief gives neither
