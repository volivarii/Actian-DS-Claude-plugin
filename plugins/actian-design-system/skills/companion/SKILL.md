...
| 9 | "make it feel like X" / "match this style" + ref URLs | yes + refs | `/generate-flow X --ref <refs>` or `/convert-to-hifi <url> --ref <refs>` |
...
- **Library lookup** (row 16: "find every empty state", "where do we use X", "show me all Y"): grep registries (`docs/dskit.json`, `docs/fmkit.json`, `docs/metakit.json`) and component guidelines, answer inline. No skill needed.
