...
`card_motion` uses its own fallback path: when the component-guideline
has no `behavior.motion.pattern` and `categoryDefaults.card_motion` has a
`patternRefs[0]`, `resolveSection` returns a Phase A result with
`fallback: true`, `fallbackReason: "category-motion-default"` and the
resolved pattern's content. The card is emitted with `_source:
"figma"`, `_fallback: true`, `_fallbackReason: "category-motion-default"`.
