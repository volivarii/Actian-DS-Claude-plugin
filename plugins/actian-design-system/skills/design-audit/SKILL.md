...
- **Copy review** (scope: `copy`, `all`): Two-layer audit. (1) For each component present in the audited target (a single component, a screen, or a multi-screen flow), read the matching per-topic file at `../../vendor/content/src/<topic-slug>.md` for component-specific rules — use `../../vendor/content/src/content-index.md` to resolve the topic-slug (e.g., Button → `buttons.md`, Modal → `modal.md`, Empty State → `empty-and-system-states.md`). For multi-component targets, read each relevant per-topic file. (2) Always apply the cross-cutting rules from `../../vendor/content/dist/content.md` (Global guidelines section): sentence case on all UI text; verb + object button labels ("Save changes" not "Save Changes" or bare "Save"); no banned words (please, sorry, ensure, execute, abort, sign in, signin, disabled); error messages state what went wrong + how to fix it; empty states include headline + body + CTA; placeholder text models expected input and never repeats the field label; terminology consistent with `vendor/app-context/app-context.json` glossary. When no per-topic file matches a component, fall back to consolidated content.md only.
