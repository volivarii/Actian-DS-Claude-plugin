...
For warning-level findings printed as CLI bracket labels (`token`, `terminology`, `avoid-word`, `unmuted-chrome`, `text-style`, `unfilled-slot`, `density-floor`, `missing-focus`, `undeclared-invention` (DS-native flows only)): exit 2, proceeds. Findings surface in the GenLog text node (and in the deliverable when pushed). Info-level grounding advisories (`chrome-ungrounded`, `chrome-divergence`, `pattern-ungrounded`, `relationships-ungrounded`, `properties-ungrounded`, `enum-not-typed`, `section-ungrounded`, `prototype-dead-end`) print the same way and never block (exit 0).
...
- `references/generate-flow/html-reference.md`: HTML template, FM components
...
- `references/generate-flow/share.md` — internals
- `references/generate-flow/research-guide.md` — research
