...
field, a card, a badge, a dialog). A multi-screen product flow is `/actian-ux-prototype`; a Figma push is
`/actian-ux-prototype --push`. A proposal never pushes: when the request says "push to Figma" or "in Figma", say
so in one line and offer `/actian-ux-prototype`. Under `--evaluate` there is no document, only the data file.

## Input shapes

| Shape | Pattern | Example |
|---|---|---|
| Request | prose, with or without a ticket already in context | `/actian-ux-proposal show a user their roles in the account menu` |
| Ticket | a ticket id, pasted text, or an attached PDF or screenshots | `/actian-ux-proposal DIP-I-496` |
...
| `--publish` | off | Publish the document as a shareable page at Step 6 without asking first; "Publishing" in `references/actian-ux-proposal/document-authoring.md` is the how. Refused together with `--evaluate`, which writes no document |
...
`references/actian-ux-proposal/document-authoring.md`, which is the whole of it: ask which of `competitors`,
...
what the ticket forces, as "The evaluation stage" in `references/actian-ux-proposal/document-authoring.md`
...
**Step 5, document.** Read `references/actian-ux-proposal/document-authoring.md`, the palette in
...
"make this a flow" (`/actian-ux-prototype --from proposals/proposal-data.json` composes every pick into one
...
- `references/actian-ux-proposal/document-authoring.md`, the research gate, the sections, the fragment
