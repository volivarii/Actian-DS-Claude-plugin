...
H1 and before the first `##` heading. The plugin-root test in the repository's test tree (outside the
plugin, so an install does not carry it) asserts each copy is identical to this one and that the bash line resolves a simulated Cowork mount, so edit here first,
