...
The table below covers the 69 authorable slugs (registry `section:"Components"`). **BUILT** = full CSS-styled HTML leaf renderer. **chip** = graceful labeled chip (`<span class="ds-component" data-slug="...">`) — still valid to author; renders as a legible placeholder.
...
| `chat-with-ai-steward` | Chat with AI Steward | chip | size / history |
...
The following 15 slugs have real HTML leaf renderers. Prop names are case-sensitive and must match exactly. Omitting a prop uses the renderer's fallback default (shown in parentheses).
...
- `variant.Type`: `Primary` (info) · `Success` · `Warning` · `Danger`. Drives the status icon (`info-filled`/`success-filled`/`warning-filled`/`error-filled`) and the role (`status`, or `alert` for `Danger`).
