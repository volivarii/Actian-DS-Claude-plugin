...
`Continue with: /actian-ux-proposal --from proposals/proposal-data.json`. `--no-research` alongside
...
- **options**: two to four. Each has an `id` (a slug), a `name`, an `anchor` (`app` slug and the
  ...
  `screens` (one to four entries in actian-ux-prototype's screen-list shape: `name`, `template` from
