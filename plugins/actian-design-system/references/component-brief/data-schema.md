...
Optional integer field on `vendor/components/dist/guidelines/<slug>.json` (typically under `domains.design.anatomy.scale` once that domain is authored). Overrides the Pattern 9 anatomy diagram's automatic scale heuristic. Valid values: 1, 2, 3, or 4.
