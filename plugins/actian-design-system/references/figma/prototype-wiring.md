...
Programmatic Figma prototype wiring via `setReactionsAsync`. Makes flow frames playable in Figma's Presentation mode. Used by the push step (actian-ux-prototype Step 7, new flows) and standalone wiring (existing flows via Figma URL).

## Two Entry Points

### Fast path (new flows, the push step: actian-ux-prototype Step 7)
