...
> `skills/actian-ux-prototype/SKILL.md` is at its 30000-byte ceiling. Anything more about the bridge
...
`form-create`, `detail-view`), and that is a different vocabulary from `/actian-ux-prototype`'s own
screen-list `template`, which names chrome (`studio`, `explorer`, `administration`, ...; see the
Flags table in `skills/actian-ux-prototype/SKILL.md`). The seed emits both: `template` carries the
screen's `app` forward, so it renders chrome the way any other actian-ux-prototype screen does, and the
...
`/actian-ux-prototype` writes at Gate 3's skeleton step: with no single app to name for the whole flow, it
...
and so no screens; resume it with `/actian-ux-proposal --from <file>` first.
