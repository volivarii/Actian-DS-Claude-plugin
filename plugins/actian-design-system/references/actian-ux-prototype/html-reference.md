...
HTML template structure, FM component rules, and Figma output details for the actian-ux-prototype skill.
...
Refer to `../../vendor/components/dist/fm-components.md` for the full Figma library inventory with component keys.
...
**Rules:**
1. **FM first** — check `../../vendor/components/dist/fm-components.md` before going custom
...
which is the single source of truth for the Figma-node → HTML mapping. It handles these
`node.type` values:
...
| `goto` | string (a screen id) | FRAME and INSTANCE nodes only; a prototype click target (`data-goto` in the render), see below |
...
A FRAME or INSTANCE content node may carry `goto: "<screen id>"`, a click target that navigates the
...
the **default, first-class deliverable** emitted on every actian-ux-prototype run. No
...
See `../figma/annotation-reference.md` for full details. Inlining ensures annotations work in all environments (CLI, Desktop, any browser).
...
Does NOT need detaching — `contentArea.appendChild()` works. See `../../vendor/components/dist/meta-kit/components.md`.
...
3. **Bind library tokens** — follow `../figma/figma-output.md` § "Token binding". Hex fallback only.
