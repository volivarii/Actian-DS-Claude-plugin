...
The image→structure fingerprint-extraction pipeline used only when `actian-ux-prototype`
is given a reference (`--ref <url>` populating `meta.references[]`). Loaded on demand
from `skills/actian-ux-prototype/SKILL.md` (Pipeline step 4.5). Opt-in: skipped entirely
