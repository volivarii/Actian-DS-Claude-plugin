# actian-ux-prototype — push opt-in model
...
Want to do more?
  ...
  push + audit   — push to Figma, then run /actian-ux-audit on the result
...
- "audit" / "push + audit" / "push+audit" / "push and audit" → push first,
  then audit the pushed Figma result. (`/actian-ux-audit` takes a Figma URL, so
