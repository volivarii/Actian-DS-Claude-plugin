...
Detailed detection rules and the step-by-step refine behavior for `actian-ux-prototype`'s
...
| Refine on a hifi unit | n/a | Re-route through `/actian-ux-prototype` refine on the underlying lo-fi data model (hifi is downstream); explain to designer. |
