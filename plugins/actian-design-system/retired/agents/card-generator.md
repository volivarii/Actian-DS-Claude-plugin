...
- **Example copy:** Any example/specimen text inside generated cards (button labels, error messages, placeholder text, table cells, tooltips) must comply with `vendor/content/dist/global.md` (cross-cutting voice/tone) + per-component `vendor/components/dist/guidelines/<slug>.json` `domains.content` — sentence case, verb + object button labels, no banned words — apply the full avoid-list in `vendor/content/dist/words-to-avoid.json` (do not inline a subset), realistic placeholders that model input rather than repeating the field label.
